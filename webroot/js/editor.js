...
(function( $ ){
	...
	var methods = {
		...
		createMenuItem: function(itemSetting, options, returnElement){
			...
			if(itemSetting["select"]){
				...
				menuWrapElement.append($('<a/>',{
										...
										"href":"javascript:void(0)",
										"title":itemSetting["tooltip"]
										}).html(itemSetting["default"]).append($("<span/>",{class:"caret"})).mousedown(function(e){
											e.preventDefault();
										}));
		    ...
		    else if(itemSetting["modal"]){
		    	var menuWrapElement = methods.createModal.apply(this,[itemSetting["modalId"], itemSetting["modalHeader"], itemSetting["modalBody"], itemSetting["onSave"]]);		    			    	
		    	var menuElement = $("<i/>");
		    	if(itemSetting["icon"])
					menuElement.addClass(itemSetting["icon"]);
				else
					menuElement.html(itemSetting["text"]);
				...
				}(this, methods,itemSetting["beforeLoad"]));
				menuWrapElement.attr('title', itemSetting['tooltip']);
			...
			if(itemSetting["custom"]){
				menuWrapElement.bind(action, (function(obj, params){
						return function(){
						methods.saveSelection.apply(obj);
						itemSetting["custom"].apply(obj, [$(this), params]);
						}
					})(this, itemSetting['params']));
			}
			else{
				menuWrapElement.data("commandName", itemSetting["commandname"]);

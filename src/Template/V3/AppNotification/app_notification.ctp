    ...
    $('#html-template').keydown(function(e){
                  ...
                  $.each(data, function(key,json){
                     ...
                     i = i + 1; 
                         ...
                         html += '<tr><td>' + 
                                  ...
                                  '<div class="check-style"><input type="checkbox" value="None"  name="subscribers" id="select_'+ i +'" class="terms-check" checked/><label for="select_'+ i +'"> </label></div></td>'+
                               // '<input id="select_'+ i +'" type="checkbox"  checked></td>'+
                                '<input type="text" style="display:block" value="'+json.apnId +'" id="user_gcm_'+i+'">'+
                                '<input type="hidden" value="'+json.userId +'" id="user_id_'+i+'">'+
                              '<td gcm="'+json.gcmId +'" apn="'+json.apnId +'" id="user_name_'+ i +'" >'+json.fullName +'</td>' +
                                '<td>'+json.email +'</td></tr>';
                     i = i + 1;    
     ...
     $('#select').on('click', function(event){
        ...
        for(i = 0; i < count; i++){
            ...
            var apn = $("#user_name_"+i).attr('apn');
            var client = i + 1;
            if(gcm){
          user_list += '<div id="close_'+i+'" class="user-list-preview">'+name+
               '<input style="display: none" name="client_a-'+ client +'" id="client-1" type="text" value="'+ gcm +'">' +
               ...
               '<button onclick="removeme(this, event);" class="remove-user"><span class="fa fa-close"></span></button></div>';  
                ...
                user_list += '<div id="close_'+i+'" class="user-list-preview">'+name+
               '<input style="display: none" name="client_i-'+ client +'" id="client-1" type="text" value="'+ apn +'">' +
               '<input name="userId-'+ client +'" type="hidden" value="'+ uid +'">' +
               '<button onclick="removeme(this, event);" class="remove-user"><span class="fa fa-close"></span></button></div>';  
